...
```shell
ubuntu@ubuntu:~$ sudo netstat -tap | grep mysql #查看是否系统中已经安装mysql
ubuntu@ubuntu:~$ sudo apt-get install mysql-server mysql-client libmysqlclient-dev
```
...
```shell
ubuntu@ubuntu:~$ ps aux | grep mysql
...
ubuntu@ubuntu:~$ mysqladmin -u root -p shutdown
Enter password: 
ubuntu@ubuntu:~$ ps aux | grep mysql
jiangxin  1984  0.0  0.0  15984   964 pts/0    S+   10:46   0:00 grep --color=auto mysql
ubuntu@ubuntu:~$ sudo apt-get autoremove --purge mysql-*
...
ubuntu@ubuntu:~$ dpkg -l |grep ^rc|awk '{print $2}' |sudo xargs dpkg -P 
...
...
### 无法远程访问MySQL服务器-服务器仅允许本地访问3306端口

无法远程访问MySQL服务器，先查看MySQL服务器是否在监听3306端口：

```shell
ubuntu@ubuntu:~$ sudo netstat -plunt | grep mysql
tcp        0      0 127.0.0.1:3306          0.0.0.0:*               LISTEN      55686/mysqld
tcp        0      0 127.0.0.1:33060         0.0.0.0:*               LISTEN      55686/mysqld
```

可以看到MySQL服务器正在监听3306端口，但是只在127.0.0.1上监听，拒绝了其他IP的访问。需要修改 /etc/mysql/mysql.conf.d/mysqld.cnf 文件
...
```shell
ubuntu@ubuntu:~$ sudo netstat -plunt | grep mysql
tcp6       0      0 :::33060                :::*                    LISTEN      56373/mysqld
tcp6       0      0 :::3306                 :::*                    LISTEN      56373/mysqld
```

### 无法远程访问MySQL服务器-root用户仅允许本地访问

在用远程连接MySQL服务器的数据库，错误信息: `ERROR 1130: Host 218.2.165.251 is not allowed to connect to this MySQL server`
...
### 无法远程访问MySQL服务器-认证方式仅允许用户为root时才能登录数据库

对于mysql 8+版本，在ubuntu上安装mysql的时候root用户默认认证方式为auth_socket。这个时候只有使用root账户登录或者使用sudo命令才能够登录数据库。
...
在某些场景下，我们可能需要远程通过root用户登录数据库，这时需要修改root用户的认证方式为mysql_native_password

```sql
-- 修改认证插件为 mysql_native_password（兼容性最好）
ALTER USER 'root'@'%' IDENTIFIED WITH mysql_native_password BY 'test';

-- 或 MySQL 8.0 默认方式（IDEA 需要较新驱动）
ALTER USER 'root'@'%' IDENTIFIED WITH caching_sha2_password BY 'test';
